...
//! Security-focused tests for RocksDB database implementation
...
fn test_path_traversal_resistance() {
    ...
    for malicious_path in malicious_paths {
        ...
        let result = RocksDbManager::new(&full_path);
...
fn test_resource_exhaustion_protection() {
    ...
    let temp_dir = TempDir::new().unwrap();
    let manager = RocksDbManager::new(temp_dir.path().to_str().unwrap()).unwrap();

    let mut collection = manager.create_collection("test", "prefix").unwrap();

    // Test large key/value pairs
    let large_key = "k".repeat(1000); // 1KB key
    ...
    match result {
        Ok(_) => {
            ...
            assert_eq!(retrieved_data.len(), large_value.len());
        }
        Err(err) => {
...
fn test_malformed_data_handling() {
    ...
    let edge_cases = vec![
        ...
        ("very_long_key_very_long_key_very_long_key_very_long_key_very_long_key_", vec![42; 1000]), // Long key/value
        ("control_chars\x01\x02\x03", vec![0x1F, 0x7F, 0x80, 0xFF]),
        ("newline_key\n\r", b"newline_value\n\r".to_vec()),
        ("tab_key\t", b"tab_value\t".to_vec()),
    ];
    ...
    for (key, value) in edge_cases {
        ...
        match result {
            Ok(_) => {
                ...
                assert!(retrieved.is_ok(), "Should be able to retrieve stored data");
...
fn test_iterator_safety() {
    ...
    assert_eq!(count, 100, "Should iterate over all items");
    ...
    assert_eq!(reverse_count, 100, "Should reverse iterate over all items");
    ...
    let range_result = Collection::get_by_range(&collection, Some("00050".to_string()), -5);
...
fn test_database_corruption_resilience() {
    ...
    let potentially_corrupting_operations = vec![
        ...
        ("very_long_key_".repeat(1000), vec![42; 10000]),
